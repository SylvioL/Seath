...
if __name__=='__main__':
    ...
    usage='usage: python %s -p 1 -d 2 -f minmean --sample_number 30 <Input_Shpfile> <Outputfile_txt>' %sys.argv[0]
    print len(sys.argv)
    if len(sys.argv) == 11:
        
        parser = OptionParser(usage=usage)
        parser.add_option("-p","--classe1", dest="Classe_1", action="store", type="string", help="Classe 1",default=None)
        parser.add_option("-d","--classe2", dest="Classe_2", action="store", type="string", help="Classe 2",default=None)
        parser.add_option("-f","--field_name", dest="Nom_du_champs_des_statistiques", action="store", type="string", help="Statistics field name",default=None)		
        parser.add_option("--sample_number", dest="Nombre_echantillons", action="store", type="float", help="Sample number",default=None)
        (options, args) = parser.parse_args()
        
        Classe_1 = options.Classe_1        
        Classe_2 = options.Classe_2
        Nom_du_champs_des_statistiques = options.Nom_du_champs_des_statistiques
        Nombre_echantillons = options.Nombre_echantillons
                        
        Shapefile_en_entree = sys.argv[9]
        Fichier_txt_de_sortie = sys.argv[10]
